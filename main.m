...
[Par_theta_r,Par_theta_z]=Par_theta_rz(Coord_R,Coord_Z,Coord_theta);
...
%%  
%% 流线迭代
% 假设初始压力梯度为0，初始压力为1个大气压
[m,n]=size(Coord_R);
p0=101325;
for i=1:1:m
    for j=1:1:n
        test1(i,j)=Coord_Wm(i,j)*Par_Wm_Par_m(i,j)*cos(Coord_gamma(i,j)+Coord_phi(i,j));
        test2(i,j)=Coord_Wm(i,j).^2*Par_phi_Par_m(i,j)*sin(Coord_gamma(i,j)+Coord_phi(i,j));
        test3(i,j)=Coord_rcu(i,j)/Coord_R(i,j).^2*sin(Coord_gamma(i,j));
        test4(i,j)=cot_epsilon(i,j);
        Par_p_Par_q(i,j)=Coord_Wm(i,j)*Par_Wm_Par_m(i,j)*cos(Coord_gamma(i,j)+Coord_phi(i,j))+...
                         Coord_Wm(i,j).^2*Par_phi_Par_m(i,j)*sin(Coord_gamma(i,j)+Coord_phi(i,j))-...
                         Coord_rcu(i,j)/Coord_R(i,j).^2*sin(Coord_gamma(i,j))+...
...
W_in=Coord_W(:,1);
H_rot=parameter.Cp*parameter.T_tot_in+W_in.^2/2-parameter.omega.^2*Coord_R(:,1).^2;
...
for i=1:1:m
    for j=2:1:n
        A(i,j)=Coord_thedyn(i,j)-Coord_thedyn(i,j-1)+parameter.R*log(Coord_P(i,j)/Coord_P(i,j-1));
...
for i=1:1:m
    for j=1:1:n
        Coord_W(i,j)=real((2*H_rot(i)-2*parameter.Cp*Coord_T_re(i,j)-(parameter.omega*Coord_R(i,j).^2)).^0.5);
    end
end
%% 
for i=1:1:m
